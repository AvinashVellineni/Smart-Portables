...
public class TrackerList extends HttpServlet {
	...
	/* Checks the Tablets type whether it is microsft or apple or samsung */
		...
		pw.print("<a style='font-size: 24px;'>" + name + " Pet Trackers</a>");
		...
		for (Map.Entry<String, Tracker> entry : hm.entrySet()) {
			...
			if (i % 1 == 1)
				...
				pw.print("<td style='font-family: Helvetica Neue,Helvetica,Arial,sans-serif;font-size: 14px;font-weight: bold;font-color: black !important;color: #d80a14;'>"+Tracker.getName()+"</td>");
				pw.print("<td id='item'><img style='padding-top: 10px;padding-bottom: 10px;margin-bottom: -3px;width: 80%;height: 150px;' src='images/PetTrackers/"
					+ Tracker.getImage() + "' alt='' /></td>");
				pw.print("<td style='font-family: initial;font-size: 14px;'>$" + Tracker.getPrice() + "</td>");
				pw.print("<form method='post' action='Cart'>" +
					"<input type='hidden' name='name' value='"+entry.getKey()+"'>"+
					"<input type='hidden' name='type' value='trackers'>"+
				...
				pw.print("<form method='post' action='WriteReview'>"+"<input type='hidden' name='name' value='"+entry.getKey()+"'>"+
					"<input type='hidden' name='type' value='trackers'>"+
					"<input type='hidden' name='maker' value='"+CategoryName+"'>"+
					"<input type='hidden' name='price' value='"+Tracker.getPrice()+"'>"+
					"<input type='hidden' name='access' value=''>"+
